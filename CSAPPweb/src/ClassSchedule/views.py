...
def classSchedule(request) :
    context = {}
    context['permission'] = request.session["permission"]
    return render(request, "ClassSchedule/classSchedule.html",context)
            ...
            if permission==2:
                ...
                for message in messages:
                    if message["message_type"]==opt:
                        data["message"][message["timeStamp"]]=message["publish_date"]+"#"+message["message_to"]+"@内容："+message["message_content"]
            elif permission==3:
                ...
                data["class_name"]=teacher_class_name.copy()
            ...
            if permission==2:
                ...
                for message in messages:
                    if message['message_to']==class_tmp:
                        data["message"][message["timeStamp"]]=message["message_to"]+"@"+message["message_content"]
            elif permission==3:
                teacher_class_name=request.session.get("teacher_class_name",[])
                teacher_class_name.append("all(自己所带的小班课的所有班)")
...
def ajaxPublish(request):
    permission=request.session.get("permission",-1)
    if request.is_ajax() and (permission in [2,3,4]):
        opt=int(request.POST['opt'])
        content=request.POST['text']
        opt_class=request.POST['opt_class']
        currenttime=time.time()
        date=time.strftime('%Y-%m-%d',time.localtime(time.time()))
        uid=request.session.get("uid",None)
        
        if opt_class=="all(自己所带的小班课的所有班)"  and permission==3:
            class_collection=request.session.get("teacher_class_name",[])
            for tmp_class in class_collection:
            ...
            Messages(timeStamp=currenttime, publish_date=date, publisher=uid,\
                message_to="all", message_type=opt, message_content=content,is_valid=True).save()
                
        else :
            Messages(timeStamp=currenttime, publish_date=date, publisher=uid,\
                message_to=opt_class, message_type=opt, message_content=content,is_valid=True).save()
...
def ajaxDelete(request):
    ...
    if request.is_ajax() and (permission in [2,3,4]):
        ...
        #刪除
        Messages.objects(timeStamp=time_stamp)[0].delete()
        return JsonResponse({"is_success":"true"})
    return JsonResponse({"is_success":"false"})