...
import com.dream.mediaplayer.views.TouchInterceptor;

public class TrackBrowserFragment extends ListFragment
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
    public static final String BUY_LICENSE = "android.drmservice.intent.action.BUY_LICENSE";
    private static final int CHILD_MENU_BASE = 100;
    ...
    private RelativeLayout relative_add_item;

    public TrackBrowserFragment()
    {
    }
    
    public TrackBrowserFragment(Bundle args)
    {
    	setArguments(args);
    }
...
//            }
            mEditMode = bundle.getString("action").equals(Intent.ACTION_EDIT);
    	}
    	...
    	View root = inflater.inflate(R.layout.queue_fragment_layout, container, false);
    	
    	currentText = (TextView) root.findViewById(R.id.currentText);
    	relative_add_item = (RelativeLayout) root.findViewById(R.id.relative_add_item);
    	relative_add_item.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent intent = new Intent(getActivity(), AddTracksToQueueActivity.class);
				getActivity().startActivity(intent);
			}
		});
    	
//    	mTrackList = getListView();
    	mTrackList = (ListView)root.findViewById(android.R.id.list);
        mTrackList.setOnCreateContextMenuListener(this);
        mTrackList.setCacheColorHint(0);
        if (mEditMode) {
            ((TouchInterceptor) mTrackList).setDropListener(mDropListener);
            ((TouchInterceptor) mTrackList).setRemoveListener(mRemoveListener);
            ((TouchInterceptor) mTrackList).registerContentObserver(getActivity().getApplicationContext());
    ...
    public void onDestroy() {
        ...
        if (lv != null) {
            ...
            if (mEditMode) {
                ...
                ((TouchInterceptor) lv).unregisterContentObserver(getActivity().getApplicationContext());
            }
        }
...
//            	setResult(RESULT_OK);
        ...
        stateIntentfilter.addAction(ApolloService.PLAYSTATE_CHANGED);
        getActivity().registerReceiver(mStatusListener, stateIntentfilter);
    }
    @Override
    public void onPause() {
...
//                MusicUtils.setSpinnerState(TrackBrowserActivity.this);
    ...
    private BroadcastReceiver mStatusListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            if (action.equals(ApolloService.PLAYSTATE_CHANGED)) {
                if (null != mAdapter)
                    getTrackCursor(mAdapter.getQueryHandler(), null,true);
...
//        setTitle();
        ...
        f.addAction(ApolloService.QUEUE_CHANGED);
        if ("nowplaying".equals(mPlaylist)) {
            try {
                int cur = MusicUtils.mService.getQueuePosition();
                setSelection(cur);
                getActivity().registerReceiver(mNowPlayingListener, new IntentFilter(f));
                mNowPlayingListener.onReceive(getActivity(), new Intent(ApolloService.META_CHANGED));
            ...
            mTrackListListener.onReceive(getActivity(), new Intent(ApolloService.META_CHANGED));
        }*/
    }
    ...
    private void updateCurrentText() {
    	if (MusicUtils.mService  != null) {
    		...
    		int totalNum = 0;
    		try {
    			curNum = MusicUtils.mService.getQueuePosition();
				totalNum = MusicUtils.mService.getQueue().length;
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
    		currentText.setText((curNum+1)+"/"+totalNum);
    	}
    }
...
//        mTrackList.setCacheColorHint(0);
    ...
    private void removePlaylistItem(int which) {
        ...
        try {
            if (MusicUtils.mService != null
                    && which != MusicUtils.mService.getQueuePosition()) {
                mDeletedOneRow = true;
            }
        } catch (RemoteException e) {
    ...
    private BroadcastReceiver mNowPlayingListener = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals(ApolloService.META_CHANGED)) {
            	...
            	mTrackList.invalidateViews();
//                MusicUtils.updateNowPlaying(TrackBrowserActivity.this);
            } else if (intent.getAction().equals(ApolloService.QUEUE_CHANGED)) {
//            	Log.e(LOGTAG, "mNowPlayingListener -1- action = "+intent.getAction());
            	updateCurrentText();
                ...
                if (mAdapter != null) {
                    Cursor c = new NowPlayingCursor(MusicUtils.mService, mCursorCols);
                    if (c.getCount() == 0) {
                    	getActivity().finish();
                        Toast.makeText(getActivity(), getResources().getString(R.string.please_select_a_track), Toast.LENGTH_SHORT).show();
                        return;
                    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfoIn) {
    	MenuColorFix.addMenu(menu, 0, PLAY_SELECTION, 0, getActivity().getResources().getString(R.string.play_all));
//        sub = menu.addSubMenu(0, ADD_TO_PLAYLIST, 0, R.string.add_to_playlist);
//        MusicUtils.makePlaylistMenu(this, sub);
        if (mEditMode) {
        	MenuColorFix.addMenu(menu, 0, REMOVE, 0, getActivity().getResources().getString(R.string.remove));
        }
...
//            }
//        } else {
//            menu.add(0, USE_AS_RINGTONE, 0, R.string.use_as_ringtone);
//        }
        
        MenuColorFix.addMenu(menu, 0, USE_AS_RINGTONE, 0, getActivity().getResources().getString(R.string.use_as_ringtone));

        MenuColorFix.addMenu(menu, 0, DELETE_ITEM, 0, getActivity().getResources().getString(R.string.real_remove));
...
//                MusicUtils.setRingtone(this, mSelectedId);
    ...
    private void removeItem() {
        ...
        if ("nowplaying".equals(mPlaylist)) {
            ...
            try {
                if (curpos != MusicUtils.mService.getQueuePosition()) {
                    mDeletedOneRow = true;
                }
            } catch (RemoteException ex) {
            }
	...
	public void onListItemClick(ListView l, View v, int position, long id)
    {
        ...
        if (mTrackCursor instanceof NowPlayingCursor) {
            if (MusicUtils.mService != null) {
                try {
                    MusicUtils.mService.setQueuePosition(position);
                    return;
                } catch (RemoteException ex) {
                }
            }
...
//            MusicUtils.setPlayListId(Long.valueOf(mPlaylist));
        ...
        /*Object[] objectArray = mAdapter.getSections();
        if (objectArray.length > 0) {
        	for (int i = 0; i < objectArray.length; i++) {
        		Log.e("aaaaaaaa", "bbb -- objectArray["+i+"] = "+objectArray[i]+", mAdapter.getSectionForPosition(i) = "+mAdapter.getSectionForPosition(i)+", " +
        				"mAdapter.getPositionForSection(i) = "+mAdapter.getPositionForSection(i));
        	}
        }*/
    }
...
//    }
    ...
    private Cursor getTrackCursor(TrackListAdapter.TrackQueryHandler queryhandler, String filter,
        ...
        if (mGenre != null) {
            ...
            if (mPlaylist.equals("nowplaying")) {
                if (MusicUtils.mService != null) {
                    ret = new NowPlayingCursor(MusicUtils.mService, mCursorCols);
                    if (ret.getCount() == 0) {
                    	getActivity().finish();
                        Toast.makeText(getActivity(), getResources().getString(R.string.please_select_a_track), Toast.LENGTH_SHORT).show();
                    }
    ...
    {
        public NowPlayingCursor(IApolloService service, String [] cols)
        {
            mCols = cols;
            mService  = service;
...
//            ImageView drm_icon;
        ...
        class TrackQueryHandler extends AsyncQueryHandler {
            ...
            public Cursor doQuery(Uri uri, String[] projection,
                ...
                return MusicUtils.query(mActivity.getActivity(),
        ...
        TrackListAdapter(Context context, TrackBrowserFragment currentactivity,
        ...
        public void setActivity(TrackBrowserFragment newactivity) {
...
//            iv.setVisibility(View.GONE);
            ...
            vh.line1 = (TextView) v.findViewById(R.id.music_title);
            vh.line2 = (TextView) v.findViewById(R.id.music_Artist);
//            vh.visualizerView = (VisualizerView) v.findViewById(R.id.visualizerView);
            vh.visualizerView = (ImageView) v.findViewById(R.id.visualizerView);
...
//            ImageView iv = vh.play_indicator;
            ...
            if (MusicUtils.mService != null) {
                ...
                try {
                    if (mIsNowPlaying) {
                        id = MusicUtils.mService.getQueuePosition();
                    } else {
                        id = MusicUtils.mService.getAudioId();
                    }
                } catch (RemoteException ex) {
                }
            }
...
//                iv.setVisibility(View.VISIBLE);
            	...
            	vh.line1.setTextColor(context.getResources().getColor(R.color.listview_item_textview_select_color));
            	vh.line2.setTextColor(context.getResources().getColor(R.color.listview_item_textview_select_color));
    			
//    			viewholder.isPlayIV.setVisibility(View.VISIBLE);
            	vh.visualizerView.setVisibility(View.VISIBLE);
    			
//    			WeakReference<VisualizerView> mView = new WeakReference<VisualizerView>(vh.visualizerView);
//    	        VisualizerUtils.updateVisualizerView(mView);
    	        
    	        vh.visualizerView.setImageResource(R.anim.visualizer_anim);
    	        AnimationDrawable animationDrawable = (AnimationDrawable)vh.visualizerView.getDrawable();
    	        try {
	                ...
	                e.printStackTrace();
	                vh.visualizerView.setVisibility(View.INVISIBLE);
	    			vh.visualizerView.setImageResource(0);
	            }
            } else {
//                iv.setVisibility(View.GONE);
            	
            	vh.line1.setTextColor(context.getResources().getColor(R.color.queue_listview_item_title_textview_unselect_color));
            	vh.line2.setTextColor(context.getResources().getColor(R.color.queue_listview_item_artist_textview_unselect_color));
