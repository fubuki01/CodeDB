...
import com.dream.mediaplayer.activity.PlayExternal;
import com.dream.mediaplayer.cache.ImageInfo;
import com.dream.mediaplayer.helpers.GetBitmapTask;
...
import com.dream.mediaplayer.helpers.utils.VaulePreference;
import com.dream.mediaplayer.helpers.utils.permissionCheck;
import com.dream.mediaplayer.preferences.SharedPreferencesCompat;
import com.dream.mediaplayer.ui.widgets.MediaAppWidgetProvider;
...
public class ApolloService extends Service implements GetBitmapTask.OnBitmapReadyListener {
    ...
    public static final String APOLLO_PACKAGE_NAME = "com.dream.mediaplayer";
    public static final String MUSIC_PACKAGE_NAME = "com.android.music";
    public static final String PLAYSTATE_CHANGED = "com.dream.mediaplayer.playstatechanged";
    public static final String META_CHANGED = "com.dream.mediaplayer.metachanged";
    public static final String FAVORITE_CHANGED = "com.dream.mediaplayer.favoritechanged";
    public static final String QUEUE_CHANGED = "com.dream.mediaplayer.queuechanged";
    public static final String REPEATMODE_CHANGED = "com.dream.mediaplayer.repeatmodechanged";
    public static final String SHUFFLEMODE_CHANGED = "com.dream.mediaplayer.shufflemodechanged";
    public static final String PROGRESSBAR_CHANGED = "com.dream.mediaplayer.progressbarchnaged";
    public static final String REFRESH_PROGRESSBAR = "com.dream.mediaplayer.refreshprogessbar";
    public static final String CYCLEREPEAT_ACTION = "com.dream.mediaplayer.musicservicecommand.cyclerepeat";
    public static final String TOGGLESHUFFLE_ACTION = "com.dream.mediaplayer.musicservicecommand.toggleshuffle";
    public static final String SERVICECMD = "com.dream.mediaplayer.musicservicecommand";
    public static final String CMDNAME = "command";
    public static final String CMDTOGGLEPAUSE = "togglepause";
    public static final String CMDALARM = "alarmcome";
    ...
    public static final String TOGGLEPAUSE_ACTION = "com.dream.mediaplayer.musicservicecommand.togglepause";
    public static final String PAUSE_ACTION = "com.dream.mediaplayer.musicservicecommand.pause";
    public static final String PREVIOUS_ACTION = "com.dream.mediaplayer.musicservicecommand.previous";
    //Action from AlarmReceiver
    public static final String ALARM_START_ACTION = "com.android.music.musicservicecommand.pause";
    public static final String ALARM_CLOSE_ACTION = "com.android.music.musicservicecommand.togglepause";
    public static final String NEXT_ACTION = "com.dream.mediaplayer.musicservicecommand.next";
...
//    private GetNowPlayingEntriesMonitor mGetNowPlayingEntriesMonitor;
    ...
    public static final String UPDATE_LRC_ACTION = "com.dream.mediaplayer.musicservicecommand.updatelrc";
...
//    private Notification status = new Notification();
    ...
    public static VaulePreference valuesave;
    //mIsStop is save the state of stop
...
//    private MediaAppWidgetProvider_4_2 mAppWidgetProvider_4_2 = MediaAppWidgetProvider_4_2.getInstance();
    ...
    private Bitmap mAlbumBlurBitmap;
            ...
            switch (msg.what) {
                ...
                case SERVER_DIED:
                    if (mIsSupposedToBePlaying) {
                        gotoNext(true);
                        Toast.makeText(ApolloService.this, R.string.playback_failed, Toast.LENGTH_SHORT).show();
                    } else {
                        // the server died when we were idle, so just
                        // reopen the same song (it will start again
                        // from the beginning though when the user
                        // restarts)
//                        Log.e("openCurrentAndNext", "handleMessage: ");
                        openCurrentAndNext();
                    }
                    break;
                ...
                case FOCUSCHANGE:
                    ...
                    switch (msg.arg1) {
                        ...
                        case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
                            Log.v(LOGTAG, "AudioFocus: received AUDIOFOCUS_DUCK");
                            if (isPlaying()) {
...
//                            Log.e("openCurrentAndNext", "handleMessage: ");
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            Log.i("ApolloService", "mIntentReceiver+耳机按钮监听");
...
//                mAppWidgetProvider.performUpdate(ApolloService.this, appWidgetIds);
//                mAppWidgetProvider_4_2.performUpdate(ApolloService.this, appWidgetIds);
    ...
    public void onCreate() {
        super.onCreate();
        Log.i("ApolloService", "onCreate");
        // add for clear the notification when the service restart
        stopForeground(true);
//        defaultAlbumBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.rootboot_bk);
...
//            mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, vol, 0);
        ...
        if (!MainActivity.startFlag) {
            ...
            Log.e("wyd", "onReceive: " + "nihaodfjsrs");
...
//        mRemoteControlClient.setNowPlayingEntriesUpdateListener(mGetNowPlayingEntriesMonitor);
        ...
        commandFilter.addAction(UPDATE_LRC_ACTION);
...
//        mControlInStatusBar = getApplicationContext().getResources().getBoolean(R.bool.control_in_statusbar);
        mControlInStatusBar = false;

        valuesave = new VaulePreference(this);
...
//        mShakeDetector.start();
    ...
    public void onDestroy() {
        ...
        if (mAlbumBlurBitmap != null) {
            mAlbumBlurBitmap.recycle();
            mAlbumBlurBitmap = null;
        }
...
//        }
        ...
        valuesave.savaGuidePosition(this, false);
    ...
    private void reloadQueue() {
        ...
        if (qlen > 1) {
            ...
            //Modified get repeat mode after reboot by liuyue 20160817 start
            int repmode = mPreferences.getInt("repeatmode", REPEAT_NONE);
            if (repmode != REPEAT_ALL && repmode != REPEAT_CURRENT) {
                repmode = REPEAT_NONE;
            }
            mRepeatMode = repmode;
            //Modified get repeat mode after reboot by liuyue 20160817 end
            ...
            mQuietMode = true;
//            Log.e("openCurrentAndNext", "reloadQueue: ");
            openCurrentAndNext();
            mQuietMode = false;
            if (mPlayer != null && !mPlayer.isInitialized()) {
...
//                    + " (requested " + seekpos + ")");
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.i("ApolloService", "onStartCommand");
        ...
        if (intent != null) {
            ...
            if (CMDALARM.equals(cmd)) {
                Log.i("ApolloService", "Alarm");
                if (action.equals(ALARM_START_ACTION)) {
                    // Log.i("SSSS","1");
                    if (!isPlaying()) {
                ...
                } else if (action.equals(ALARM_CLOSE_ACTION)) {
                    // Log.i("SSSS","2");
                    if (!mIsStop) {
                        play();
                        // reset the value of mIsStop
                        mIsStop = true;
                    }
                }
            }
                    ...
                    if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
                        if (mCursor != null && !mCursor.isAfterLast()) {
                            ...
                            if (curTrackPath != null && curTrackPath.contains(intent.getData().getPath())) {
                                Log.i("action_eject", "onReceive: xxx");
                                saveQueue(true);
...
//                                notifyChange(META_CHANGED);
                        ...
                        Log.i("action_mounted", "onReceive: xxx");
                        ...
                        //Modified not to pause music when receive mount by liuyue 20160812 start
                        // pause playback
                        /*pause();
                        reloadQueue();
                        mQueueIsSaveable = true;*/
                        //Modified not to pause music when receive mount by liuyue 20160812 end
                        notifyChange(PLAYSTATE_CHANGED);
                        notifyChange(QUEUE_CHANGED);
                        notifyChange(META_CHANGED);
                        ...
                        if (id != -1 && artindex != -1) {
                            MusicUtils.deleteTrack(ApolloService.this, id, artindex);
                        }
                    }
                }
            };
    ...
    public void registerA2dpServiceListener() {
        mA2dpReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                String action = intent.getAction();
                String cmd = intent.getStringExtra("command");
                Log.i("ApolloService", "registerA2dpServiceListener+耳机按钮监听");
...
//        mAppWidgetProvider_4_2.notifyChange(this, what);
    }
...
//                    Log.e("openCurrentAndNext", "enqueue: ");
                    openCurrentAndNext();
                    play();
                    notifyChange(META_CHANGED);
                    return;
...
//                Log.e("openCurrentAndNext", "enqueue:");
    ...
    public void open(long[] list, int position) {
        synchronized (this) {
            ...
            Log.e("nihao", "open: "+mPlayListLen+"   "+listlength);
...
//            mAppWidgetProvider_4_2.setPauseState(true);
//            Log.e("openCurrentAndNext", "open: ");
            openCurrentAndNext();
...
//            mAppWidgetProvider_4_2.setPauseState(false);
        }
    }
    ...
    private void  openCurrentAndNext() {
        //Log.e(LOGTAG, "openCurrentAndNext() this = "+this);
        synchronized (this) {
            if (mCursor != null) {
                mCursor.close();
                mCursor = null;
...
//                    Log.e("mOpenFailedCounter", "openCurrentAndNext: "+mOpenFailedCounter
//                            +"   "+mPlayListLen);
                    ...
                    Log.e("playback_failed", "mQuietMode: "+mQuietMode);
                    if (!mQuietMode) {
                        Toast.makeText(this, R.string.playback_failed, Toast.LENGTH_SHORT).show();
                        Log.e("playback_failed", "mQuietMode: ");
                    }
        ...
        synchronized (this) {
            ...
            if (mCursor == null) {
                ...
                if (path.startsWith("content://media/")) {
                    ...
                    where = null;
                    selectionArgs = null;
                    // the path is from filemanager or form 'setting->general use-> braowser'
                } else if (path.startsWith("/document/primary")) {
                    String s[] = path.split(":");
                    String trackDisplayName = s[1];
                    uri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
                    where = MediaStore.Audio.Media.DISPLAY_NAME + "=?";
                    selectionArgs = new String[]{trackDisplayName};
                } else if (path.startsWith("/document/audio")) {
                    String s[] = path.split(":");
                    uri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
                    where = MediaStore.Audio.Media._ID + "=?";
                    selectionArgs = new String[]{s[1]};
                } else {  //if the path is from bluetooth or download form e-mail or from external sdcard  or other unkown situation
                    int start = path.lastIndexOf("/");
                    String trackDisplayName = path.substring(start + 1);
                    uri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
                    if (trackDisplayName.contains(":")) {
                        trackDisplayName = trackDisplayName.substring(trackDisplayName.indexOf(":") + 1);
                        Log.i("path", "歌曲名字:" + trackDisplayName);
                    }
                    where = MediaStore.Audio.Media.DISPLAY_NAME + "=?";
                    selectionArgs = new String[]{trackDisplayName};
                }
                try {
                    ...
                    Log.e(LOGTAG, "open() -- ex = " + ex.getMessage());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
     ...
     */
    public void play() {
        // if the machine is not C3L, then judge the file type
        Log.e("play", "play: "+Build.MODEL);
        if (!(Build.MODEL.equals(Constants.MACHINE_MODE)||Build.MODEL.equals(Constants.MACHINE_MODE2))) {
            Log.e("play", "play: "+Build.MODEL);
            String trackDisplayName = getTrackDisplayName();
            if (!MusicUtils.isSupportableFileType(trackDisplayName)) {
                Toast.makeText(this, getResources().getString(R.string.file_type_not_surportable), Toast.LENGTH_SHORT).show();
                gotoNext(false);
                return;
            }
        }
        if (getAudioId() == -1 && mPlayListLen == 1) {
        ...
        if (views != null && status != null && mControlInStatusBar) {
            ...
            views.setImageViewResource(R.id.status_bar_play, (isPlaying() ?
                    R.drawable.apollo_holo_dark_pause : R.drawable.apollo_holo_dark_play));
...
//            views.setViewVisibility(R.id.status_bar_album_art, View.GONE);
            views.setImageViewResource(R.id.status_bar_album_art, R.drawable.bottom_bar_cover_default);
        }
        ...
        views.setOnClickPendingIntent(R.id.status_bar_prev, prevPendingIntent);

        Intent toggleIntent = new Intent(ApolloService.TOGGLEPAUSE_ACTION);
        PendingIntent togglePendingIntent = PendingIntent.getBroadcast(this,
                0  no requestCode , toggleIntent, 0  no flags );
        views.setOnClickPendingIntent(R.id.status_bar_play, togglePendingIntent);
        ...
        if (mControlInStatusBar) {
            views.setViewVisibility(R.id.status_bar_exit, View.VISIBLE);
            views.setImageViewResource(R.id.status_bar_exit, R.drawable.exit);
            ...
            views.setOnClickPendingIntent(R.id.status_bar_exit, exitPendingIntent);
        }

        if (getAudioId() < 0) {
            // streaming
            views.setTextViewText(R.id.status_bar_track_name, getPath());
        ...
        status.icon = isPlaying() ? R.drawable.notification_pause_icon : R.drawable.notification_play_icon;
//        status.icon = R.drawable.bottom_bar_cover_default;
        status.contentIntent = PendingIntent.getActivity(this, 0,
                new Intent("com.dream.mediaplayer.PLAYBACK_VIEWER")
...
//                }
                ...
                if (mControlInStatusBar) {
                    ...
                    views.setOnClickPendingIntent(R.id.status_bar_play, playPendingIntent);
                    startForeground(PLAYBACKSERVICE_STATUS, status);
                }
            }
        }
    ...
    public void prev() {
        synchronized (this) {
            if (mPlayListLen == 1 && getAudioId() == -1) {
                int shuffleMode = getShuffleMode();
                setShuffleMode(SHUFFLE_AUTO);
                setShuffleMode(shuffleMode);
            }
...
//            Log.e("openCurrentAndNext", "prev: ");
    ...
    public void gotoNext(boolean force) {
        //Log.e(LOGTAG, "gotoNext() this = "+this);
        synchronized (this) {
            if (getAudioId() == -1 && mPlayListLen == 1) {
                int shuffleMode = getShuffleMode();
                setShuffleMode(SHUFFLE_AUTO);
...
//            mAppWidgetProvider_4_2.setPauseState(true);
            stop(false);
            mPlayPos = pos;
//            Log.e("openCurrentAndNext", "gotoNext: ");
            openCurrentAndNext();
//            mAppWidgetProvider.setPauseState(false);
//            mAppWidgetProvider_4_2.setPauseState(false);
            play();
        ...
        synchronized (this) {
            ...
            if (gotonext) {
                if (mPlayListLen == 0) {
                    ...
                    boolean wasPlaying = isPlaying();
                    stop(false);
//                    Log.e("openCurrentAndNext", "removeTracksInternal: ");
                    openCurrentAndNext();
                    if (wasPlaying) {
                        play();
                    }
        ...
        synchronized (this) {
            ...
            if (mShuffleMode == SHUFFLE_AUTO) {
                if (makeAutoShuffleList()) {
                    ...
                    doAutoShuffleUpdate();
                    mPlayPos = 0;
//                    Log.e("openCurrentAndNext", "setShuffleMode: ");
                    openCurrentAndNext();
                    play();
                    notifyChange(META_CHANGED);
                    notifyAttributeValues(PLAYERSETTINGS_RESPONSE,
...
//            notifyChange(PLAYSTATE_CHANGED);
    ...
    public long getAudioId() {
        synchronized (this) {
            if (mPlayPos >= 0 && mPlayer != null && mPlayer.isInitialized() && mPlayList.length > 0) {
    ...
    public void setQueuePosition(int pos) {
        synchronized (this) {
            stop(false);
            mPlayPos = pos;
//            Log.e("openCurrentAndNext", "setQueuePosition: ");
            openCurrentAndNext();
            play();
            notifyChange(META_CHANGED);
            if (mShuffleMode == SHUFFLE_AUTO) {
    ...
    public Bitmap getAlbumBlurBitmap() {
        synchronized (this) {
            return mAlbumBlurBitmap;
        }
    }
    ...
    public String getTrackDisplayName() {
        synchronized (this) {
            ...
            return mCursor.getString(mCursor.getColumnIndexOrThrow(MediaStore.Audio.Media.DISPLAY_NAME));
        }
    }
    ...
    static class ServiceStub extends IApolloService.Stub {
        ...
        public Bitmap getAlbumBlurBitmap() {
            return mService.get().getAlbumBlurBitmap();
        }
    ...
    private class MultiPlayer {
        ...
        public void setDataSource(String path) {//wyd
            if (!(MainActivity.startFlag||PlayExternal.startFlag)) {
                path = null;
                Log.e("wyd", "setDataSource: " + "niah"+mIsComplete);
            } else {
        ...
        private boolean setDataSourceImpl(final MediaPlayer player, String path) {
            ...
            try {
                ...
                if (path.startsWith("content://")) {
                    ...
                    String musicInfo[] = path.split(":");
                    player.setDataSource(ApolloService.this, Uri.parse("/mnt/sdcard/" + musicInfo[1]));
                } else if (path.startsWith("/document/audio")) {    // if the path is startWith "/document/audio:id"
                    String musicInfo[] = path.split(":");
                    player.setDataSource(ApolloService.this, Uri.parse("content://media/external/audio/media/" + musicInfo[1]));
                } else {   // if the path is from bluetooth or other unkonw situation
                    int start = path.lastIndexOf("/");
                    String trackName = path.substring(start + 1);
                    if (trackName.contains(":")) {
                        trackName = trackName.substring(trackName.indexOf(":") + 1);
                    }
                    Cursor c = getContentResolver().query(
                            MediaStore.Audio.Media.EXTERNAL_CONTENT_URI,
                            mCursorCols, MediaStore.Audio.Media.DISPLAY_NAME + "=?",
                            new String[]{trackName}, null);
                    if (c != null) {
                        c.moveToFirst();
                    }
                    int trackId = c.getInt(c.getColumnIndexOrThrow(MediaStore.Audio.Media._ID));
                    player.setDataSource(ApolloService.this, Uri.parse("content://media/external/audio/media/" + trackId));
                }
                player.setAudioStreamType(AudioManager.STREAM_MUSIC);
                player.prepare();
            } catch (IOException ex) {
                Log.e(LOGTAG, "setDataSourceImpl() - ex = " + ex.getMessage());
                if (!mQuietMode && !isNextPlayer) {
                    if(mytoast==null)
                        mytoast=Toast.makeText(ApolloService.this, R.string.open_failed, Toast.LENGTH_SHORT);
                    else
                    {
                        mytoast.cancel();
                        mytoast=Toast.makeText(ApolloService.this, R.string.open_failed, Toast.LENGTH_SHORT);
                ...
                //Log.e(LOGTAG, "setDataSourceImpl() - e = "+e.getMessage());
                if (!mQuietMode && !isNextPlayer) {
                    if(mytoast==null)
                        mytoast=Toast.makeText(ApolloService.this, R.string.open_failed, Toast.LENGTH_SHORT);
                    else
