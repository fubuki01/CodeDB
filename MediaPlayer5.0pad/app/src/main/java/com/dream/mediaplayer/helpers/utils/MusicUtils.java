...
public class MusicUtils {
    ...
    public static int getCardId(Context mContext) {
        ...
        if (c != null) {
            c.moveToFirst();
            if(c.getCount()==0)
                Toast.makeText(mContext, "未开启储存权限或没有找到歌曲", Toast.LENGTH_SHORT).show();
            else
                id = c.getInt(0);
            c.close();
        }
...
//                }
            ...
            mService.open(list, force_shuffle ? -1 : position);//xxx
            mService.play();
    ...
    /*public static void setRingtone(Context context, long id, int sub_id) {
        ContentResolver resolver = context.getContentResolver();
...
//					}
    ...
    /**
     * 功能：选择歌曲，添加到播放列表中
     * @param context :为了得到查询数据库的context
     * @param ids：点击要添加歌曲的ID
     * @param playlistid：选择添加到对应歌曲列表的ID
     */
    public static void addToPlaylist(Context context, long[] ids, long playlistid) {
        ...
        if (ids == null) {
            ...
            int size = ids.length;
            long[] newids = new long[size] ; //为了后面删除ids中与列表中重复的歌曲ID
            ContentResolver resolver = context.getContentResolver();  //得到内容提供者（安卓四大组件）

            String[] cols = new String[] {   //统计对应的列表中有多少首歌曲
                    "count(*)"
            };
            String[] cols2 = new String[] {       //以歌曲的ID进行查询音乐数据库
                    Playlists.Members.AUDIO_ID
            };
            Uri uri = Audio.Playlists.Members.getContentUri(EXTERNAL, playlistid); //找到对应查询的路径即数据库表名
            Cursor cur = resolver.query(uri, cols2, null, null, null);
            if (cur == null) {    //没有查询到对应的表
                return;
            }
            cur.moveToFirst();  //移动游标到查询表中的第一行
            //判断是否当前添加的歌曲已经存在对应的列表中（防止重复添加）这里通过歌曲的ID来判断的
            while (!cur.isAfterLast()) {
                for(int index =0 ; index <ids.length ;index++) {
                    if (cur.getLong(0) == ids[index]) {    //如果存在有重复的歌曲ID，就不添加到需要添加歌曲数组中
                        ids[index] = -1 ; //表示这个歌曲对应的ID，已经存在，用-1表示存在
                        break;
                    }
                }
                cur.moveToNext();  //移动数据库查询游标
            }
            //得到需要真正添加歌曲的ID
            int addNumber = 0;
            for(int out = 0 ;out <size ;out++){
                if(ids[out] != -1){
                    newids[addNumber] = ids[out];
                    addNumber++;
                }
            }
            int base = cur.getCount();  //获取到对应列表中已经存在多少首歌曲
            cur.close();                //关闭游标资源
            if (addNumber  == 0){  //表示没有要添加的歌曲，即点击添加的歌曲都是已经存在的
                String message = "歌曲已存在对应列表或未添加歌曲";
                Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
                return;
            }
            int numinserted = 0;  //记录该次添加歌曲的数目
            for (int i = 0; i < size; i += 1000) {          //插入歌曲
                makeInsertItems(newids, i, 1000, base);       //给新添加的歌曲分配对应位置的ID
                numinserted += resolver.bulkInsert(uri, sContentValuesCache);//批量或者单个添加数据到对应的播放列表
            }

            if (numinserted > 0) {  //添加歌曲数目大于0
                String message = context.getResources().getQuantityString(
                        R.plurals.NNNtrackstoplaylist, addNumber, addNumber); //addNumber表示这次添加的歌曲数目
                Toast.makeText(context, message, Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private static void makeInsertItems(long[] ids, int offset, int len, int base) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            sContentValuesCache[i].put(Playlists.Members.PLAY_ORDER, base + offset + i); //插入的位置
            sContentValuesCache[i].put(Playlists.Members.AUDIO_ID, ids[offset + i]);  //插入的列表ID
        }
    }
    ...
    public static void addToCurrentPlaylist(Context mContext, long[] list) {
        ...
        if (mService == null)
            return;
        try {
            List<Long> allMusicID = new ArrayList<Long>();  //存储当前列表中的歌曲ID
            List<Long> newAddMusic= new ArrayList<Long>();  //存储需要添加的歌曲ID

            long[] name = mService.getQueue();  //拿到当前播放列表中的歌曲数目的ID
            for(int i = 0 ; i<name.length ; i++){
                allMusicID.add(name[i]);        //拿到当前播放列表中的I歌曲D
            }
            //比较之前的ID与现在需要加入的ID的歌曲是否重复
            for(int m = 0 ; m < list.length ;m++){
                if(!allMusicID.contains(list[m])){   //ID不重复，则需要进行添加新歌曲
                    newAddMusic.add(list[m]);
                }
            }
            //将List转为数组,进行添加
            long[] add = new long[newAddMusic.size()];
            int n = 0;
            for (long id: newAddMusic) {
                add[n] = id;
                n++;
            }
            mService.enqueue(add, ApolloService.LAST);   //将新添加歌曲的ID加入队列中
            if(add.length == 0){  //表示添加的歌曲要么已经存在，要么添加的数目为0，两种情况
                Toast.makeText(mContext, "未添加歌曲或添加的歌曲已经存在", Toast.LENGTH_SHORT).show();
            }
            else {
                String message = mContext.getResources().getQuantityString(
                        R.plurals.NNNtrackstoplaylist, add.length, add.length);  //成功添加的歌曲数目
