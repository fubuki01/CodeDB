...
//            mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, vol, 0);
        ...
        if(!MainActivity.havePermission)
            ;
        else
...
//            iFilter.addAction(Intent.ACTION_MEDIA_MOUNTED);
            filter.addAction(ACTION_DELETE_MUSIC);
//            iFilter.addDataScheme("file");
            ...
            filter.addAction(Intent.ACTION_MEDIA_BUTTON);  //完全拔出
...
//            mAppWidgetProvider_4_2.setPauseState(false);
     ...
     */
    public boolean open(String path) {  //xxx
        synchronized (this) {
            if (path == null) {
                return false;
            }
     ...
     */
    public void play() {    //xxx
        // if the machine is not C3L, then judge the file type
        if(!Build.MODEL.equals(Constants.MACHINE_MODE)) {
        ...
        if(getAudioId() == -1 && mPlayListLen == 1) {
            ...
            //setShuffleMode(SHUFFLE_AUTO);//xxx
        ...
        mAudioManager.registerMediaButtonEventReceiver(new ComponentName(this.getPackageName(),
                MediaButtonIntentReceiver.class.getName()));
        mAudioManager.registerRemoteControlClient(mRemoteControlClient);
        if (mPlayer != null && mPlayer.isInitialized()) {//xxx
...
//            notifyChange(PLAYSTATE_CHANGED);
    ...
    private class MultiPlayer {
        ...
        private boolean mIsInitialized = false;//xxx
        ...
        public void setDataSource(String path) {    //xxx
            if(MainActivity.havePermission || PlayExternal.havePermission)
        ...
        private boolean setDataSourceImpl(final MediaPlayer player, String path) {
            ...
            try {
                ...
                if (!mQuietMode && !isNextPlayer) {
                    Toast.makeText(ApolloService.this, R.string.open_failed, Toast.LENGTH_SHORT).show();
                }
                return false;
