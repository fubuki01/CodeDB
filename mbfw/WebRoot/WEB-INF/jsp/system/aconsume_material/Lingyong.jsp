...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
				<form action="asset/acm_use.do" method="post" name="savely" id="savely">
					<!-- 检索  -->
				<!-- 检索  -->
				<c:if test="${permission != 3}">
						...
						<tr>
						<td style="vertical-align:top;"><a class="btn btn-small btn-success" onclick="add();">新增耗材领用表</a></td>	
						<td style="vertical-align:top;"><a class="btn btn-small btn-danger"  onclick="deleteLy('确定要删除选中的数据吗?');" "  >批量删除</a></td>																				
					...
					<table id="table_report" 
						...
						<thead>
							<tr>
								...
								<th>实际数量</th>
								<th>领用人</th>
								<th><i class="icon-time hidden-phone"></i>领用部门</th>
								<th>领用公司</th>
								<th>发放人</th>
								<th><i class="icon-time hidden-phone"></i>领用时间</th>
                                <th>备注</th>
                                <c:if test="${permission != 3}">
								...
								<c:when test="${not empty listApprover}">
									<c:forEach items="${listApprover}" var="supplies"
										...
										<label><input type='checkbox' name='ids' value="${supplies.id}" id="" alt=""/><span class="lbl"></span></label>
											...
											<td>${supplies.leader}</td>
											<td>${supplies.department}</td>
											<td>${supplies.company}</td>
											<td>${supplies.release_people}</td>
											<td>${supplies.time}</td>
											<td><a>${supplies.remarks}</a></td>
											...
											<td style="width: 60px;">
												<div class="hidden-phone visible-desktop btn-group">
													...
													<a class="btn btn-mini btn-info" title="编辑"
														onclick="editLy(${supplies.id});"><i
														class="icon-edit"></i></a> <a class="btn btn-mini btn-danger"
														title="删除"
														onclick="delLy('${supplies.id}','${supplies.supplies_name}');"><i
...
<!-- 					<button class="btn btn-primary" data-dismiss="modal" onclick="deleteLy();">确定</button> -->
...
<!-- 	</div> -->
	<!-- --------模态框处理结束部分------------ -->			
		...
		//删除操作
		function delLy(supply_id , msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>asset/delLingyong_add.do?id="+supply_id;
					$.get(url,function(data){							
						// nextPage(${page.currentPage});
						window.location = "${pageContext.request.contextPath}/asset/acm_use";
					});
				}
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="耗材領用表";
			 diag.URL = '<%=basePath%>asset/Lingyong_add.do';
			 diag.Width = 780;
			 diag.Height = 420;
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>asset/delete_Ly.do?tm='+new Date().getTime(),
						    	data: {id:str},
								dataType:'json',
...
// 		//批量操作
// 		function deleteLy(){
// 			$('#alldataform').attr({action:"${pageContext.request.contextPath}/asset/delete_Ly"});
// 			$('#alldataform').submit();
// 		} 
		...
		function editLy(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改耗材领用信息";
			 diag.URL = '<%=basePath%>asset/EditLy.do?id='+id;
			 diag.Width = 800;
			 diag.Height = 400;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/acm_use?currentPage="+clickpage+"&showCount="+${page.showCount};
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/acm_use?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
