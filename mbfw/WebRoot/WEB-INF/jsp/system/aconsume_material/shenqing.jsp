...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
				<form action="asset/acm_apply.do" method="post" name="savely" id="savely">
					<!-- 检索  -->
					<c:if test="${permission != 3}">
					<table>

						<tr>
						<td style="vertical-align:top;"><a class="btn btn-small btn-success" onclick="add();">新增耗材申请表</a></td>	
						<td style="vertical-align:top;"><a class="btn btn-small btn-danger" onclick="deleteSq('确定要删除选中的数据吗?');" >批量删除</a></td>																				
								...
								<c:when test="${not empty listApprover}">
									<c:forEach items="${listApprover}" var="supplies"
										...
										<label><input type='checkbox' name='ids' value="${supplies.id}" id="" alt=""/><span class="lbl"></span></label>
											...
											<td>${supplies.time_apply}</td>
											<td style="color: red;">${supplies.state}</td>
											...
											<td style="width: 60px;">
												<div class="hidden-phone visible-desktop btn-group">
													...
													<a class="btn btn-mini btn-info" title="编辑"
														onclick="editSq(${supplies.id});"><i
														class="icon-edit"></i></a> 
													
													<c:choose>
														<c:when test="${ supplies.state == '已提交'}">
															<a class="btn btn-mini btn-danger" title="删除" onclick="delSq('${supplies.id}','${supplies.supplies_name}');"><i
															class="icon-trash"></i></a>
														</c:when>
														<c:otherwise>
															<a class="btn btn-mini btn-default" title="由于处于审核中或者审核完成状态，无法删除!" ><i
														class="icon-trash"></i></a>
														</c:otherwise>
													</c:choose>													
												</div>
		...
		//删除操作
		function delSq(supply_id , msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>asset/delSq.do?id="+supply_id;
					$.get(url,function(data){							
						// nextPage(${page.currentPage});
						window.location = "${pageContext.request.contextPath}/asset/acm_apply";
					});
				}
		...
		function add(){
			window.location.href='<%=basePath%>asset/shenqing_add.do';
		}
...
// 			 diag.Drag=true;
// 			 diag.Title ="耗材申请表";
<%-- 			 diag.URL = '<%=basePath%>asset/shenqing_add.do'; --%>
// 			 diag.Width = 880;
// 			 diag.Height = 620;
...
// 		} 
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>asset/delete_Sq.do?tm='+new Date().getTime(),
						    	data: {id:str},
								dataType:'json',
		...
		function editSq(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改耗材申请信息";
			 diag.URL = '<%=basePath%>asset/EditSq.do?id='+id;
			 diag.Width =  880;
			 diag.Height = 620;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/acm_apply?currentPage="+clickpage+"&showCount="+${page.showCount};
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/acm_apply?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
