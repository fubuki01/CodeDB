...
<div class="container-fluid" id="main-container">
	...
	<div class="row-fluid">
			...
			<div class="row-fluid">
					...
					<!-- 检索 -->
					<form action="asset/borrow.do" method="post" name="BorrowForm" id="BorrowForm">
					<c:if test="${pd.userPermission != 2}">
						<table>
							<tr>
								<td style="vertical-align: top;"><a
									class="btn btn-small btn-success" onclick="addAllot();">新增申请表</a> 
									</td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">调入申请单名 </th>	
						<th class="center">公司名称</th>				
						<th class="center">调出部门</th>
						<th class="center">调入部门</th>
						<th class="center"><i class="icon-time hidden-phone"></i>调拨时间</th>						
						<th class="center">资产原使用（管理）人</th>
						<th class="center">资产接收（使用）人</th>
						<!-- <th class="center">审批部门（人）</th>
						<th class="center">审批意见</th> -->
						<th class="center">审批流程</th>
						<th class="center">审批状态</th>
						<th class="center">调拨原因</th>
				...
				<!-- 开始循环 -->	
					...
					<c:when test="${not empty allotList}">
						...
						<c:forEach items="${allotList}" var="allot" varStatus="vs">
							...
							<tr >
								...
								<td class="center">${allot.allot_name}</td>		
								<td class="center">${allot.bank_name}</td>	
								<td class="center">${allot.ot_department}</td>					
								<td class="center">${allot.it_department}</td>
								
								
								<td class="center">${allot.allot_time}</td>								
								<td class="center">${allot.asset_user}</td>
								<td class="center">${allot.asset_receiver}</td>
								<%-- <td class="center">${allot.rt_agency}</td> --%>
								<td class="center">${allot.apply_procedure}</td>
								<td class="center" style="color: red;">${allot.status}</td>
								<td class="center">${allot.allot_reason}</td>
								
								 <td style="width: 30px;" class="center">
								 <div class="hidden-phone visible-desktop btn-group">
								 <button class="btn btn-mini btn-info" data-toggle="dropdown"><i class="icon-cog icon-only"></i></button>
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">														
										<c:if test="${pd.userPermission <= 2}">	<li><a style="cursor:pointer;" title="编辑" onclick="editAllot('${allot.id}');" class="tooltip-success" data-rel="tooltip" data-placement="left"><span class="green"> <i class="icon-edit"></i></span></a></li>		</c:if>	
										...
										<c:if test="${pd.userPermission == 1 }"><li><a style="cursor:pointer;" title="删除" onclick="delAllot('${allot.id}','${allot.asset_code}');" class="tooltip-error" data-rel="tooltip" data-placement="left"><span class="red"><i class="icon-trash"></i></span></a></li>	</c:if>
...
<!-- 返回顶部  -->
		...
		function addAllot(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增申请表";
			 diag.URL = '<%=basePath%>asset/goAddBorrow.do';
			 diag.Width = 1050;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
		...
		function addAllot1(){
			 ...
			 diag.Drag=true;
			 diag.Title ="审批申请表";
			 diag.URL = '<%=basePath%>asset/goAddBorrow1.do';
			 diag.Width = 1050;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function editAllot(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="修改申请表";
			 diag.URL = '<%=basePath%>asset/goEditAllot.do?id='+id;
			 diag.Width = 1050;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//审批
		function editAllot1(id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="请进行审批";
			 diag.URL = '<%=basePath%>asset/goEditAllot1.do?id='+id;
			 diag.Width = 1050;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		//删除
		function delAllot(user_id,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>asset/deleteAllot.do?id='+user_id;
					$.get(url,function(data){
					 	window.location = "${pageContext.request.contextPath}/asset/borrow"; 
					});
				}
			});
		}
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>asset/deleteAllAllot.do?tm='+new Date().getTime(),
						    	data: {id:str},
								dataType:'json',
	...
	<%-- 	
		...
		function toExcel(){
			...
			window.location.href='http://localhost:8080/mbfw/happuser/excel.do?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID+'&STATUS='+STATUS;
		}
		//下面都是分页处理的操作
		//(1)操作1：点击页码进行显示数据
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/borrow?currentPage="+clickpage+"&showCount="+${page.showCount};	
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/borrow?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
