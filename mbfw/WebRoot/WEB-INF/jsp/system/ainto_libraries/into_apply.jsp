...
<html lang="en">
	...
	<link rel="stylesheet" type="text/css" href="static/js/myconfirm/css/xcConfirm.css" />
	<script src="static/js/myconfirm/js/jquery-1.9.1.js" type="text/javascript" charset="utf-8"></script>
	<script src="static/js/myconfirm/js/xcConfirm.js" type="text/javascript"charset="utf-8"></script>
		<script type="text/javascript">
		function add_asset_info(){
			window.location.href='asset/apl_asset_into_house.do';
		}
	</script>
	</head> 
<body>
...
<div id="page-content" class="clearfix">
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="asset/apl_ruku_apply.do" method="post" name="intoLibraryForm" id="intoLibraryForm">
			<table>
				<tr>
					<td style="vertical-align:top;">
					<c:if test="${QX.add ==1 }">
					<a class="btn btn-small btn-success" onclick="add();">新增入库单</a>
					</c:if>
					<c:if test="${QX.del ==1 }">
			...
			<table id="table_report" class="table text-table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
						<c:forEach items="${into_library_apply}" var="ila" varStatus="vs">
							<tr>
								<td><label><input type="checkbox" id="plsc" name="plsc" value="${ila.id}@${ila.into_purchase_bill}@${ila.into_status}" /><span class="lbl"></span></label></td>
								<td>${ vs.index+1}</td>
								<td><a>${ila.into_status}</a></td>
								<td>${ ila.into_code}</td>
								<td>${ ila.into_purchase_bill}</td>
								<td><a>${ ila.into_device}</a></td>
								<td>${ ila.into_count}</td>
								<td>${ ila.into_person}</td>
								<td>${ ila.into_time}</td>
								<td>${ ila.into_house}</td>
								<td><a>${ ila.asset_total_money}</a></td>
								<td>${ ila.into_note}</td>
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="editIntoLibraryBill('${ila.id }','${ila.into_status }');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
											<li><a style="cursor:pointer;" title="删除" onclick="delIntoLibraryBill('${ila.id }','${ila.into_purchase_bill }','${ila.into_device }','${ila.into_status }');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</ul>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="入库单";
			 diag.URL = '<%=basePath%>asset/apl_into_apply.do';
			 diag.Width = 900;
			 diag.Height = 460;
			 diag.CancelEvent = function(){ //关闭事件
		...
		//修改
		function editIntoLibraryBill(id,status){
			if(status == '未入库'){
				 top.jzts();
				 var diag = new top.Dialog();
				 diag.Drag=true;
				 diag.Title ="修改入库单";
				 diag.URL = '<%=basePath%>asset/edit_into_library_apply.do?id='+id;
				 diag.Width = 900;
				 diag.Height = 530;
				 diag.CancelEvent = function(){ //关闭事件
					 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
						//nextPage(${page.currentPage});
						 window.location = '<%=basePath %>asset/apl_ruku_apply'; 
					}
					diag.close();
				 };
				 diag.show();
		...
		//删除
		function delIntoLibraryBill(into_library_bill_id,purchase_code,msg,status){
			if(status != '未入库'){
				var txt=  "处于 [ "+status+ "状态 ]的采购单不能删除！";
				window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
			}else{
				bootbox.confirm('确定要删除供应商名称[ <b style="color: red">'+msg+" </b>]吗?", function(result) {
					if(result) {
						top.jzts();
						var url = "<%=basePath%>asset/delete_into_library_bill.do?id="+into_library_bill_id+'&purchase_code='+purchase_code;
						...
						if(str4.length == 0 || str4 == ''){
							if(msg == '确定要删除选中的数据吗?'){
								top.jzts();
								 $.ajax({
									type: "POST",
									url: '<%=basePath%>asset/delete_all_into_library_bill.do',
							    	data: {del_into_library_bill:str},
									dataType:'json',
									cache: false,
									success: function(data){
										 window.location.href = '<%=basePath %>asset/apl_ruku_apply.do?deleteResult=success';
									}
								}); 
							}
						 }else{
							 var txt=  "["+str4+ "]的入库单不能删除！";
								window.wxc.xcConfirm(txt, window.wxc.xcConfirm.typeEnum.info);
						 }
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/apl_ruku_apply?currentPage="+clickpage+"&showCount="+${page.showCount};	
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/apl_ruku_apply?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
