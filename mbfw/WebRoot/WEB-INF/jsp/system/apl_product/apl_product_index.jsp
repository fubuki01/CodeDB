...
<div id="page-content" class="clearfix">
	<div  class="row-fluid" ><!--  style="position:absolute; height:400px; overflow:auto"  -->
			...
			<!-- 检索  -->
			<form action="product/apl_product_index.do" method="post" name="productForm" id="productForm">
			<table>
				<tr>
					<td style="vertical-align:top;">
					<a class="btn btn-small btn-success" onclick="add();">新增产品</a>
			...
			<table id="table_report" class="table text-table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty productFind}">
						<c:forEach items="${productFind}" var="pf" varStatus="vs">
							<tr>
								<td><label><input type="checkbox" name="plsc" id="product_id" value="${pf.id }@${pf.product_name }@${pf.product_class }@${pf.provider_name }@${pf.product_code}"/><span class="lbl"></span></label></td>
								<td>${vs.index+1}</td>
								<td>${ pf.product_class}</td>
								<td>${ pf.product_name}</td>
								<td>${ pf.provider_name}</td>
								<td>${ pf.product_flag }</td>
								<td>${ pf.product_price}</td>
								<td>${ pf.product_unit}</td>
								<td>${ pf.product_others}</td>
								<td>${ pf.product_note}</td>
								<td style="width: 30px;" class="center">
									<div class='hidden-phone visible-desktop btn-group'>
										...
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="upProduct('${pf.product_code }','${pf.product_name}','${pf.product_total}','${pf.provider_name}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:choose>
											<c:when test="${user.USERNAME=='admin'}"></c:when>
											<c:otherwise>
												<c:if test="${QX.del == 1 }">
												<li><a style="cursor:pointer;" title="删除" onclick="delProduct('${pf.product_code }','${pf.id }','${pf.product_name }','${pf.product_class }','${pf.provider_name }');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
												</c:if>
											</c:otherwise>
											</c:choose>
										</ul>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//判断项目删除是否成功
		var delFaile = '${delFaile}';	
		if(delFaile == "faile"){
			var txt=  "该产品已信息被使用，不能删除！";
		...
		var codeRepeat = '${codeRepeat}';// 判断产品是否存在
		if(codeRepeat=="true"){
			alert("产品已存在，添加失败！");
		}
		
		var msg_product="${msg_product}";
		var product_name="${product_name}";
		if(msg_product.length > 0){
			bootbox.confirm('产品名称[ <b style="color: red">'+product_name+" </b>]已经被使用，不能删除！", function(result) {
			}
			);
		}
		var others="${others}";
		if(others=='others'){
			bootbox.confirm('<b style="color: red">该产品名称已经被使用，不能修改！</b>', function(result) {
			}
			);
		}
		//新增
		...
		//修改
		function upProduct(product_code,name,total,provider_name){
			if(total > 0 ){
				if(provider_name !='不祥'){
					bootbox.confirm('产品名称[ <b style="color: red">'+name+" </b>]已经被使用,不能修改！", function(result) {});
				}else{// 能修改供应商，其他配置，备注。其他的不能修改
					window.location.href='<%=basePath%>product/apl_product_update.do?product_code='+product_code+'&onlyedit=onlyedit';
				}
			}else{
				window.location.href='<%=basePath%>product/apl_product_update.do?product_code='+product_code+'&others=others';
			}
		}
		
		//删除
		function delProduct(product_code,product_id,msg1,msg2,msg3){
			var del='del';
			var str=msg1+"@"+msg2+"@"+msg3;
			bootbox.confirm('你确定眼删除产品[ <b style="color: red">'+msg1+" </b>]吗？", function(result) {
				window.location.href="<%=basePath%>product/delete_product.do?id="+product_id+"&product="+encodeURI(encodeURI(str))+"&product_code="+product_code;
			}
			);
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>product/delete_all_product.do',
						    	data: {del_all_product:str},
								dataType:'json',
								cache: false,
								success: function(data){
									if(data.product == null){
										 window.location = '<%=basePath %>product/apl_product_index?delResult=success';
									}else{
										 window.location = '<%=basePath %>product/apl_product_index?product='+encodeURI(encodeURI(data.product));
									}
								}
							});
						}
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/product/apl_product_index?currentPage="+clickpage+"&showCount="+${page.showCount};	
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/product/apl_product_index?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
