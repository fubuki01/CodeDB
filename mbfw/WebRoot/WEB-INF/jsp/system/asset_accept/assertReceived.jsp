...
<body>
 ...
 <div id="page-content" class="clearfix"> 
   ...
   <div class="row-fluid">
		<form action="asset/arda_assertReceived.do"  name="assetReceivedForm" id=assetReceivedForm method="post">
			<table>
				<tbody>
				<tr>
					<td>
			...
			<table id="table_report" class="table text-table table-striped table-bordered table-hover">
				...
				<tbody>
				<c:forEach items="${asset_for_received}" var="afi" varStatus="vs">
					<tr>
						...
						<td>${afi.asset_nod}</td>
						<td >
							<a  class="btn btn-mini btn-info" onclick="asset_received('${afi.id}','${afi.asset_code}');">接收</a>
						</td>
				</tr>							
			</c:forEach>
			</tbody>
...
<script type="text/javascript">
		...
		// 新增资产下发
		function asset_received(id,asset_code){
			top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="资产接收";
			 diag.URL = '<%=basePath%>asset/arda_add_received_bill_index.do?id='+id+'&asset_code='+asset_code;
			 diag.Width = 500;
			 diag.Height = 430;
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/arda_assertReceived?currentPage="+clickpage+"&showCount="+${page.showCount};	
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/arda_assertReceived?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
