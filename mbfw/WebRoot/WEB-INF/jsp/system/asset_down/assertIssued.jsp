...
<body>
 ...
 <div id="page-content" class="clearfix"> 
   ...
   <div class="row-fluid">
		<form action="asset/arda_add_assertIssued.do"  name="assetIssuedForm" id="assetIssuedForm" method="post">
			<table>
				<tbody>
				<tr>
					<td>
					...
					<td style="vertical-align:top;">
						<a  class="btn btn-mini btn-info" onclick="asset_issued_close();">后退</a>
					</td>
				</tr>
				</tbody>
			</table>
			...
			<table id="table_report" class="table text-table table-striped table-bordered table-hover">
				...
				<tbody>
				<c:forEach items="${asset_for_issued}" var="afi" varStatus="vs">
					<tr>
						<td><label><input type="checkbox" name="ids" id="ids" value="${afi.id}"><span class="lbl"></span></label></td>
						<td>${vs.index+1}</td>
						<td>${afi.asset_code}</td>
						<td>${afi.asset_name}</td>
						<td>${afi.asset_class}</td>
						<td>${afi.asset_status}</td>
						<td>${afi.asset_unit}</td>
						<td>${afi.asset_price}</td>
						<td>${afi.asset_provider}</td>
						<td>${afi.asset_storehouse}</td>
						<td>${afi.asset_purchase_time}</td>
						<td>${afi.asset_brand}</td>
						<td>${afi.asset_max_years}</td>
						<td>${afi.asset_standard_model}</td>
						<td>${afi.asset_sn}</td>
						<td>${afi.asset_detail_config}</td>
						<td>${afi.asset_nod}</td>
						<td >
							<a  class="btn btn-mini btn-info" onclick="asset_issued('${afi.id}','${afi.asset_code}');">下发</a>
						</td>
				</tr>							
			</c:forEach>
			</tbody>
...
<script type="text/javascript">
		...
		function search(){
			top.jzts();
			$("#assetIssuedForm").submit();
		}
		

		// 新增资产下发
		function asset_issued(id,asset_code){
			window.location.href='<%=basePath%>asset/arda_add_issue_bill_index.do?id='+id+'&asset_code='+asset_code;
		}
			<%-- top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="资产下发单";
			 diag.URL = '<%=basePath%>asset/arda_add_issue_bill_index.do?id='+id+'&asset_code='+asset_code;
			 diag.Width = 500;
			 diag.Height = 430;
		...
		function updateRegisterAsset(id){
			window.location.href='<%=basePath%>asset/arda_toEdit.do?id='+id;
		}
		
		// 后退
		function asset_issued_close(){
			...
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					...
					$.get(url,function(data){
					 	window.location = "${pageContext.request.contextPath}/asset/arda_assertRegister"; 
					});
				}
			});
		}
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								...
								success: function(data){
									window.location = "<%=basePath %>asset/arda_assertRegister"; 
								}
							});
						}
					}
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/arda_add_assertIssued?currentPage="+clickpage+"&showCount="+${page.showCount};	
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/arda_add_assertIssued?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
