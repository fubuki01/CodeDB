...
<body>
 ...
 <div id="page-content" class="clearfix"> 
	 <div class="row-fluid"> 
	 	<form action="asset/arda_assertRegister.do"  name="registerAssetForm" id="registerAssetForm" method="post">
	 		<table>
				<tr>
					<td style="vertical-align:top;">
					<c:if test="${QX.add ==1 }">
					<a  class="btn btn-small btn-success" onclick="add_asset_info();">新增登记资产</a>
					</c:if>
					<c:if test="${QX.del ==1 }">
				...
				<tbody>
						<c:forEach items="${asset_register}" var="assetRegister" varStatus="vs">
								<tr>
									...
									<td>${assetRegister.asset_use_company}</td>
									<td>${assetRegister.asset_use_dept}</td>
									<td>${assetRegister.asset_price}</td>
									<td>${assetRegister.asset_use}</td>
									...
									<td>${assetRegister.asset_manager}</td>
									<td>${assetRegister.asset_user}</td>
...
<script type="text/javascript">
		...
		function add_asset_info(){
			window.location.href='<%=basePath%>asset/arda_asset_into_house.do';
		}
			...
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					...
					$.get(url,function(data){
					 	window.location = "${pageContext.request.contextPath}/asset/arda_assertRegister?deleteResult=success"; 
					});
				}
			});
		}
		...
		function fromExcel(){
			 ...
			 diag.URL = '<%=basePath%>asset/arda_goUploadExcel.do';
			 diag.Width = 300;
			 diag.Height = 150;
		...
		function changePage(clickpage){	
			...
			window.location = "${pageContext.request.contextPath}/asset/arda_assertRegister?currentPage="+clickpage+"&showCount="+${page.showCount};	
		}
		//操作2：点击上下页，进行页码内容的改变
		...
		function changeCount(pagesize){	
			window.location = "${pageContext.request.contextPath}/asset/arda_assertRegister?currentPage="+${page.currentPage}+"&showCount="+pagesize;
		}
		//操作4：处理跳转按钮页面的处理
		function jumpPage(){
			//1.获取页码框中的数值大小
