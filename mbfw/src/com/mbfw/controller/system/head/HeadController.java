package com.mbfw.controller.system.head;
...
import com.mbfw.util.SmsUtil;
import com.mbfw.util.Tools;
import com.mbfw.util.Watermark;
import com.mbfw.util.mail.SimpleMailSender;

/**
 * 类名称：HeadController 创建人：研发中心 创建时间：2014年8月16日
 * 
 * @version
 */
@Controller
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception {
		...
		Tools.writeFile(Const.EMAIL, pd.getString("SMTP") + ",mbfw," + pd.getString("PORT") + ",mbfw," + pd.getString("EMAIL") + ",mbfw," + pd.getString("PAW")); // 写入邮件服务器配置
		Tools.writeFile(Const.SMS1, pd.getString("SMSU1") + ",mbfw," + pd.getString("SMSPAW1")); // 写入短信1配置
		Tools.writeFile(Const.SMS2, pd.getString("SMSU2") + ",mbfw," + pd.getString("SMSPAW2")); // 写入短信2配置
		mv.addObject("msg", "OK");
	...
	public ModelAndView saveSys2() throws Exception {
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM, pd.getString("isCheck1") + ",mbfw," + pd.getString("fcontent") + ",mbfw," + pd.getString("fontSize") + ",mbfw," + pd.getString("fontX") + ",mbfw," + pd.getString("fontY")); // 文字水印配置
		Tools.writeFile(Const.IWATERM, pd.getString("isCheck2") + ",mbfw," + pd.getString("imgUrl") + ",mbfw," + pd.getString("imgX") + ",mbfw," + pd.getString("imgY")); // 图片水印配置
		Watermark.fushValue();
		mv.addObject("msg", "OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception {
		...
		Tools.writeFile(Const.WEBSOCKET, pd.getString("WIMIP") + ",mbfw," + pd.getString("WIMPORT") + ",mbfw," + pd.getString("OLIP") + ",mbfw," + pd.getString("OLPORT")); // websocket配置
		mv.addObject("msg", "OK");
		mv.setViewName("save_result");
		return mv;
	}
