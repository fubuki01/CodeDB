package com.mbfw.controller.system.role;
...
public class RoleController extends BaseController {
	...
	public ModelAndView button(@RequestParam String ROLE_ID, @RequestParam String msg, Model model) throws Exception {
		ModelAndView mv = this.getModelAndView();
		try {
			List<Menu> menuList = menuService.listAllMenu();//获取所有的菜单项内容，表sys_menu
			Role role = roleService.getRoleById(ROLE_ID); //获取当前系统角色的权限内容，表sys_role

			String roleRights = "";
			if ("add_qx".equals(msg)) { //添加权限
				roleRights = role.getADD_QX();
			} else if ("del_qx".equals(msg)) { //删除权限
				roleRights = role.getDEL_QX();
			} else if ("edit_qx".equals(msg)) { //编辑权限
				roleRights = role.getEDIT_QX();
			} else if ("cha_qx".equals(msg)) {  //查询权限
				roleRights = role.getCHA_QX();
			}

			if (Tools.notEmpty(roleRights)) {  //如果不为空，就说明是进行了权限操作的管理设置
				for (Menu menu : menuList) {
					menu.setHasMenu(RightsHelper.testRights(roleRights, menu.getMENU_ID()));
					if (menu.isHasMenu()) {
						List<Menu> subMenuList = menu.getSubMenu();//获取主菜单下面的所有子菜单
						for (Menu sub : subMenuList) { //设置对应子菜单的权限
							sub.setHasMenu(RightsHelper.testRights(roleRights, sub.getMENU_ID()));
						}
					}
				}
			}
			JSONArray arr = JSONArray.fromObject(menuList);//转为JSON字符串
			String json = arr.toString();
