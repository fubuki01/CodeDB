...
public class ClassExcelRead {
	 ...
	 * @return list
	 */
	public static List<Object> assetClassReadExcel(String filepath, String filename, int startrow, int startcol, int sheetnum) {
	...
	public static List<Object> mertialClassReadExcel(String filepath, String filename, int startrow, int startcol, int sheetnum) {
		List<Object> varList = new ArrayList<Object>();
			...
			for (int i = startrow; i < rowNum; i++) { // 行循环开始
				...
				for (int j = startcol; j < cellNum; j++) { // 列循环开始
					...
					if( i == 2){
						if(j==0 && !"编号".equals(cellValue)){
							return null;
						}else if(j==1 && !"耗材类别".equals(cellValue)){
							return null;
						}else if(j==2 && !"耗材名称".equals(cellValue)){
							return null;
						}
					}
					else{ //因为第二行的标题不需要加到数据库中
