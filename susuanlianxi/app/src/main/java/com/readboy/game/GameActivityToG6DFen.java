...
public  abstract class GameActivityToG6DFen extends BaseGameActivity{

	protected myview game_content;
	protected boolean is_pre=false;

	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.game_view_fen_prexiao);
		findView();//获取游戏界面
		initView();
		CountTimeThread();
			...
			if(err <= 0){
				...
				for (int i = 0; i < result.length; i++)
				{
					if(answerStr.equalsIgnoreCase(result[i])){
						...
						problem+=answerStr;
						if (is_pre==false){
							String op = FAO.tostr(problem+"%");
							game_content.settext(op,answerStr.length(),false);
							Log.e("6666666666666", op+"");
				...
				if(!correct){
					problem+=result[0];
					if (is_pre==false){
						String yu = FAO.tostr(problem+"%");
						Log.e("8888888888", result[0].length()+"");
						game_content.settext(yu,result[0].length(),false);
						game_content.invalidate();
					}else {
						String yu = FAO.tostr(problem);
						game_content.settext(yu,result[0].length(),false);
